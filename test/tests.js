...
describe('Objektiv', function(){
  ...
  describe('Object property', function(){
    ...
    describe('when property exists', function() {
      ...
      it('sets a value of a property', function() {
        assert.deepEqual(Objektiv.attr('someField')(data, 2), { someField: 2, someValue: 2 });
      });

      it('modifies a value of a property', function() {
        var incr = function(x){ return x + 1};
        assert.deepEqual(Objektiv.attr('someField').mod(data, incr), { someField: 2, someValue:2 });
      });
    });
    ...
    describe('for an object property', function() {
      ...
      describe('when property not found', function() {
        it('returns undefined on get', function() {
          assert.equal(Objektiv.partialAttr('someValue').partialAttr('deep').get(data), undefined);
        });
        it('returns unchanged copy on an Object on set', function() {
          assert.deepEqual(Objektiv.partialAttr('not_found').set(data, 1), { someField: 1, someValue: 2 });
        });
      });
    });
  ...
  describe('Composition', function() {
    ...
    it('Composes two lenses', function() {
      assert.deepEqual(1,  Objektiv.attr('someField').then(Objektiv.at(0))(data));
    });

    it('Composes arbitrary number of lenses', function() {
      var composition = Objektiv.attr('someField').then(Objektiv.at(2), Objektiv.attr('foo'))
      assert.deepEqual(10,  composition(data));
    });

    it('has a shortcuts', function() {
      assert.deepEqual(1,  Objektiv.attr('someField').at(0)(data));
      assert.deepEqual(10, Objektiv.attr('someField').at(2).attr('foo')(data));
    });
  });
  ...
  describe('Data cursor', function() {
    ...
    it('receives new state and old state in a callback', function() {
      ...
      var deepCursor = Objektiv.dataCursor(data).attr('deep').attr('data');
      deepCursor.mod(function (x) {return x + 1});
    });
